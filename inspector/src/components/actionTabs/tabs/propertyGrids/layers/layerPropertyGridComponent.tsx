        ...
        return (
            <div className="pane">
                <LineContainerComponent title={getTrans('GENERAL')} selection={this.props.globalState}>
                    <TextLineComponent label={getTrans('Class')} value={layer.getClassName()} />
                    <TextInputLineComponent lockObject={this.props.lockObject} label={getTrans('Name')} target={layer} propertyName="name" onPropertyChangedObservable={this.props.onPropertyChangedObservable}/>
                    <SliderLineComponent label={getTrans('Intensity')} 
                        target={layer} propertyName="intensity"
                        minimum={0} maximum={2} step={0.01} decimalCount={2} 
                        onPropertyChangedObservable={this.props.onPropertyChangedObservable} />       
                   <SliderLineComponent label={getTrans('BlurKernelSize')} 
